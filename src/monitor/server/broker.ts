...
import { UnixSocketAdapter } from './adapters/unix-socket';
...
/**
 * Pending sub-agent spawn tracking
 * Used for time-based correlation to link sub-agents to parent sessions
 */
...
/**
 * Active subagent context tracking
 * Used to track events that occur within a Task tool call (subagent)
 * Since Claude Code uses the same session_id for parent and subagent,
 * we create virtual child sessions to group subagent events
 */
...
export class EventBroker {
  private adapter: IPCAdapter | null = null;
  ...
  private pendingSubagentSpawns: PendingSubagentSpawn[] = [];
  ...
  private activeSubagentStack: Map<string, ActiveSubagent[]> = new Map();
  ...
  private startSubagentContext(event: MonitorEvent): void {
    const toolInput = event.data.toolInput as Record<string, unknown> | undefined;
   ...
   */
  private getActiveSubagent(sessionId: string): ActiveSubagent | undefined {
  ...
  private trackSubagentSpawn(event: MonitorEvent): void {
    ...
    if (
      ...
      this.pendingSubagentSpawns.push({
        ...
        agentType: toolInput?.subagent_type as string | undefined,
      });
   ...
   */
  private directoriesShareProject(dir1: string, dir2: string): boolean {
    // Normalize paths
  ...
  private correlateSubagentSpawn(
    ...
    workingDirectory: string,
    timestamp: number
  ): { parentSessionId: string; agentType?: string } | undefined {
    ...
    const matchIndex = this.pendingSubagentSpawns.findIndex((spawn) => {
      ...
      if (!this.directoriesShareProject(spawn.workingDirectory, workingDirectory)) return false;
      // Must be within the correlation window
      if (timestamp - spawn.timestamp > SUBAGENT_CORRELATION_WINDOW_MS) return false;
      // Must not already be the same session
      if (spawn.parentSessionId === newSessionId) return false;
      return true;
    });
   ...
   */
  private cleanupPendingSpawns(): void {
    const now = Date.now();
    this.pendingSubagentSpawns = this.pendingSubagentSpawns.filter(
      (spawn) => now - spawn.timestamp < SUBAGENT_CORRELATION_WINDOW_MS * 2
    );
  }
