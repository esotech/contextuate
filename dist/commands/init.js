...
async function findLegacyBootstrapFiles() {
    ...
    for (const file of LEGACY_BOOTSTRAP_FILES) {
        ...
        if (stat.isSymbolicLink()) {
            const target = await fs_extra_1.default.readlink(file);
...
async function initCommand(platformArgs, options) {
    ...
    try {
        ...
        console.log(chalk_1.default.blue('[INFO] Creating primary instruction file...'));
        ...
        if (legacyBootstrapFiles.length > 0) {
            console.log(chalk_1.default.yellow('[WARN] Existing AI bootstrap files detected:'));
            for (const file of legacyBootstrapFiles) {
                console.log(chalk_1.default.yellow(`  - ${file}`));
            }
            console.log(chalk_1.default.yellow('[WARN] Review these files and merge any unique rules into AGENTS.md.'));
        }
        console.log('');
        ...
        if (platformsWithSymlinks.length > 0) {
            console.log(chalk_1.default.blue('[INFO] Creating platform asset symlinks...'));
        ...
        console.log(chalk_1.default.green('╚════════════════════════════════════════╝'));
        console.log('');
        console.log(`Primary instruction file: ${chalk_1.default.cyan(PRIMARY_PLATFORM_FILE.dest)}`);
        console.log('');
        console.log('Installed platforms:');
        for (const platform of selectedPlatforms) {
            const details = platform.symlinks ? 'shared AGENTS.md + platform assets' : 'shared AGENTS.md';
            console.log(`  - ${chalk_1.default.cyan(platform.name)} (${details})`);
        }
        console.log('');
